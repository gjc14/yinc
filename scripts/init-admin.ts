    ...
    try {
        ...
        if (!admin) {
            ...
            const newAdmin = await db.transaction(async tx => {
                ...
                console.log('\n管理員使用者已建立 (Admin user created):', admin)

                await tx.insert(schema.seosTable).values(defaultSEOs)
                console.log(
                    '\nSEO 已建立 (SEOs created):',
                    defaultSEOs.map(seo => ({
                        route: seo.route,
            ...
            const newAdmin = await db.transaction(async tx => {
                ...
                console.log(
                    '\n✔️ 預設文章已建立 (Default post created):',
                    postCreated.title
                )
                ...
                const tags = await db
                    ...
                    .returning()
                console.log(
                    '\n✔️ 預設標籤已建立 (Default tags created):',
                    defaultTags.map(tag => ({
                        name: tag.name,
                    }))
                )
                ...
                const categories = await db
                    ...
                    .returning()
                console.log(
                    '\n✔️ 預設分類已建立 (Default categories created):',
                    defaultCategories.map(category => ({
                        name: category.name,
                    }))
                )
