...
export const areDifferentPosts = (
    ...
    if (firstPost.content !== secondPost.content) {
        try {
            const firstEditorContent = firstPost.content
                ? generateText(JSON.parse(firstPost.content), [
                      ...ExtensionKit(),
                  ])
                : ''
            const secondEditorContent = secondPost.content
                ? generateText(JSON.parse(secondPost.content), [
                      ...ExtensionKit(),
                  ])
                : ''
    ...
    if (
        areValuesDiff(
            JSON.stringify(firstPost.seo),
            JSON.stringify(secondPost.seo)
        )
    ) {
        const seoProps = [
            'metaTitle',
    ...
    if (difference.length > 0) {
        import.meta.env.DEV &&
            console.log('[DEV] Different posts in:', difference)
        return true
    }
